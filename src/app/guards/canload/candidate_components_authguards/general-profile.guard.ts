...
@Injectable()
export class GeneralProfileGuard implements CanLoad, CanActivate {
  ...
  canLoad(
    ...
    if (this.appConfig.getLocalData('csrf-login') && this.appConfig.getLocalData('roles') == 'candidate') {
      if (this.appConfig.getSelectedCustomerCode() && this.appConfig.getSelectedCustomerCode() != '#LTTS' && this.appConfig.getSelectedCustomerCode() != '#ADANI') {
        return true;
      }else {
