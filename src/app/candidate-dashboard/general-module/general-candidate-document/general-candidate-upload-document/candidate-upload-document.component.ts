...
@Component({
  selector: 'app-general-candidate-upload-document',
...
export class GeneralCandidateUploadDocumentComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  goToProfile() {
    this.appConfig.routeNavigation(CONSTANT.ENDPOINTS.CANDIDATE_DASHBOARD.GENERAL_JOINING);
  }
  ...
  getDocuments() {
      ...
      this.candidateService.newGetProfileData(apiData).subscribe((data: any)=> {
        ...
        this.selectedPost = data && data.document_details && data.document_details.selected_post ? data.document_details.selected_post : null;
  ...
  patchBankingArray(data) {
    return this.fb.group({
      ...
      [this.form_ifsc_code]: [data[this.form_ifsc_code], [RemoveWhitespace.whitespace(), Validators.required, this.glovbal_validators.alphaNum50()]],
  ...
  initBankingArray() {
    return this.fb.group({
      ...
      [this.form_ifsc_code]: [null, [RemoveWhitespace.whitespace(), Validators.required, this.glovbal_validators.alphaNum50()]],
