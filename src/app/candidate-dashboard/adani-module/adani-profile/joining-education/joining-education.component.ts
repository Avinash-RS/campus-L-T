...
@Component({
  selector: 'app-adani-joining-education',
...
export class AdaniJoiningEducationComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    if (this.educationForm.valid && this.selectedPost) {
      ...
      if (entryValid.valid) {
       ...
       this.newSaveProfileDataSubscription = this.candidateService.newSaveProfileData(EducationApiRequestDetails).subscribe((data: any)=> {
        ...
        this.appConfig.nzNotification('error', 'Not Submitted', entryValid?.value?.label == 'gct' ? '12th or Diploma and Undergraduate are mandatory' : entryValid?.value?.label == 'pgct' ? '12th or Diploma, Undergraduate and Postgraduate are mandatory' : entryValid?.value?.label == 'det' ? 'Diploma is mandatory' : 'CA or ICWA or CS is mandatory');
      }
    } else {
      ...
      if (route == 'dependent') {
        ...
        if(this.candidateService.getLocalsection_flags() && this.candidateService.getLocalsection_flags().education_details == '1') {
         ...
         if (this.educationForm.valid) {
          return this.sharedService.openJoiningRoutePopUp.next(route == 'dependent' ? CONSTANT.ENDPOINTS.CANDIDATE_DASHBOARD.ADANI_JOINING_DEPENDENT : CONSTANT.ENDPOINTS.CANDIDATE_DASHBOARD.ADANI_JOINING_WORK);
         }
          this.glovbal_validators.validateAllFormArrays(this.educationForm.get([this.form_educationArray]) as FormArray);
          this.ngAfterViewInit();
          this.appConfig.nzNotification('error', 'Not Saved', 'Please fill all the red highlighted fields to proceed further');
