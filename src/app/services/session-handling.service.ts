...
export class NgIdleService implements OnDestroy {
  ...
  constructor(private zone: NgZone, private appConfig: AppConfigService) {
  ...
  get lastAction(): number {
    return parseInt(this.appConfig.getLocalData(STORE_KEY), 10);
  }

  set lastAction(value) {
    this.appConfig.setLocalData(STORE_KEY, value.toString());
  }
  ...
  public removeActionFromStore(): void {
    this.appConfig.clearLocalDataOne(STORE_KEY);
  }
